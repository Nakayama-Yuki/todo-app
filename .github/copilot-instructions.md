...
- **設定**: `vitest.config.ts`、`playwright.config.ts`（tsconfig パス エイリアス `@` 対応）
...
```bash
pnpm test              # 単体テスト: ウォッチモード（開発中）
pnpm test --run        # 単体テスト: シングルラン + カバレッジ（CI/CD）
pnpm exec playwright test        # E2Eテスト: Playwright実行
pnpm exec playwright show-report # E2Eテスト: レポート表示
```
