...
src/
  app/              # Next.js App Router (デフォルトではサーバー側でレンダリング)
    ...
    api/
      todos/
        route.ts    # CRUD エンドポイント (GET/POST/PUT/DELETE)
        [id]/
...
- **バリデーション**: POST/PUT ハンドラーでの入力検証 (`route.ts` を参照)
...
- **配置**: `tests/unit/` ディレクトリ内にソースのディレクトリ構造を反映して配置
- **例**: `tests/unit/components/AddTask.test.tsx`、`tests/unit/app/api/todos/route.test.ts`、`tests/unit/lib/db.test.ts`
- **テストランナー**: Vitest（jsdom 環境、React Testing Library 使用）
- **設定**: `vitest.config.ts` に基づく（tsconfig パス エイリアス `@` 対応）

#### E2E テスト

- **配置**: `tests/e2e/` ディレクトリ内に `*.spec.ts` として配置
